...
function command ( args, cb ) {
	...
	var nodeCommand = args[0]
		, nodeArgs = args[1];
		console.log(nodeCommand);
	IO.jsonp({
		url : 'http://127.0.0.1:1337/' + nodeCommand + (nodeArgs ? '?' + nodeArgs : ''),
		jsonpName : 'callback',
		fun : finish
	});
	...
	function finish ( resp ) {
		...
		if (resp.err) {
			res = 'Error: ' + resp.err + ' happened, blame @SomeKittens';
			bot.log(resp.err);
		} else {
...
bot.addCommand({
	...
	description : 'I am a great magician!',
	async : true
});
})();
